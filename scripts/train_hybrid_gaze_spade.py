...
backbone, guided by segmentation masks. This creates a hybrid model where
the main features are from DenseNet, but the semantic context for SPADE
modulation comes from DINOv2.
"""
import os
import sys
...
try:
    ...
    from src.dinov2_backbone import DinoV2Backbone # NEW: Import for DINO features
...
_logger = logging.getLogger("train_deepgaze_dynamic_dino_hybrid")
...
class DeepGazeIIIDynamicDinoEmbedding(nn.Module):
    def __init__(self, features_module: FeatureExtractor, dino_features_module: DinoV2Backbone,
        ...
        # Ensure DINO backbone is frozen as it's only a feature extractor here
        if hasattr(self.dino_features, 'parameters'):
            for param in self.dino_features.parameters():
                param.requires_grad = False
        if hasattr(self.dino_features, 'eval'):
    ...
    def forward(self, image, centerbias, segmentation_mask, **kwargs):
        ...
        # Path 1: DINOv2 for semantic feature extraction
        with torch.no_grad(): # Explicit no_grad for frozen backbone
            dino_feature_maps = self.dino_features(img_for_features)
            # The DINO backbone is configured to return only one layer's features
            semantic_dino_patches = dino_feature_maps[0]
        
        # Create the painted map using DINO features and the segmentation mask
        painted_map = self._create_painted_semantic_map(semantic_dino_patches, segmentation_mask)

        # Path 2: DenseNet for main saliency feature extraction
        ...
        # Modulate DenseNet features with the DINO-based painted map
    ...
    def train(self, mode=True):
        ...
        if hasattr(self.dino_features, 'eval'): self.dino_features.eval() # DINO is always in eval mode
...
def main(args: argparse.Namespace):
    ...
    dino_semantic_backbone = DinoV2Backbone(
        layers=[args.dino_semantic_layer_idx], # We only need one layer from DINO
        model_name=args.dino_model_name,
        patch_size=args.dino_patch_size,
        freeze=True
    ).to(device)
    dino_semantic_backbone.eval()
    ...
    # STAGE DISPATCH: SALICON PRETRAINING
    if args.stage.startswith('salicon_pretrain'):
        if is_master: _logger.info(f"--- Preparing SALICON Pretraining ---")
        experiment_name = f"{args.stage}_{args.densenet_model_name}_dino_{args.dino_model_name}"
        ...
        train_ll_cache = args.dataset_dir/f'salicon_baseline_train_ll_{args.densenet_model_name}_dino_hybrid.pkl'
        val_ll_cache = args.dataset_dir/f'salicon_baseline_val_ll_{args.densenet_model_name}_dino_hybrid.pkl'
        ...
        model = DeepGazeIIIDynamicDinoEmbedding(features_module, dino_semantic_backbone, saliency_net, fixsel_net, args.num_total_segments, args.finalizer_learn_sigma, args.finalizer_initial_sigma).to(device)
    ...
    # STAGE DISPATCH: MIT FINE-TUNING
        ...
        experiment_name_mit = f"{args.stage}_fold{fold}_{args.densenet_model_name}_dino_{args.dino_model_name}_k{args.num_total_segments}_lr{current_lr}"
        
        salicon_checkpoint_path = args.salicon_checkpoint_path
        if not salicon_checkpoint_path:
            salicon_dir = args.train_dir / f"{args.stage.replace('mit_spatial', 'salicon_pretrain')}_{args.densenet_model_name}_dino_{args.dino_model_name}"
        ...
        train_ll_cache = args.dataset_dir / f'mit1003_baseline_train_ll_fold{fold}_{args.densenet_model_name}_dino_hybrid_converted.pkl'
        val_ll_cache = args.dataset_dir / f'mit1003_baseline_val_ll_fold{fold}_{args.densenet_model_name}_dino_hybrid_converted.pkl'
        train_ll, val_ll = None, None
        if is_master:
            try: train_ll = cpickle.load(open(train_ll_cache, 'rb'))
            except: train_ll = centerbias.information_gain(train_stim, train_fix, verbose=True, average='image'); cpickle.dump(train_ll, open(train_ll_cache, 'wb'))
        ...
        # Instantiate model on CPU first for safe checkpoint loading
        model_cpu = DeepGazeIIIDynamicDinoEmbedding(features_module.cpu(), dino_semantic_backbone.cpu(), saliency_net, fixsel_net, args.num_total_segments, args.finalizer_learn_sigma, args.finalizer_initial_sigma)
        
        # Move backbones back to GPU for training
        ...
        if salicon_checkpoint_path and salicon_checkpoint_path.exists():
            ...
            _logger.warning(f"No SALICON checkpoint found at '{salicon_checkpoint_path}'. Starting MIT fine-tuning from scratch.")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(parents=[_pre], description="Train DeepGaze with Dynamic SPADE Normalization using DINO for semantic embeddings.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)

    # --- Model & Stage Configuration ---
    parser.add_argument('--stage', choices=['salicon_pretrain_densenet_dino_hybrid', 'mit_spatial_densenet_dino_hybrid'], help='Training stage to execute.')
    parser.add_argument('--log_level', type=str, default='INFO', choices=['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL'])
    
    # --- DenseNet Backbone (Main Path) ---
    parser.add_argument('--densenet_model_name', default='densenet201', choices=['densenet201'])

    # --- DINOv2 Backbone (Semantic Path) ---
    parser.add_argument('--dino_model_name', default='dinov2_vitb14', choices=['dinov2_vitb14', 'dinov2_vitl14', 'dinov2_vitg14'], help="DINOv2 model for semantic feature extraction.")
    parser.add_argument('--dino_patch_size', type=int, default=14, help="Patch size of the DINOv2 model.")
    ...
    parser.add_argument('--train_dir', type=str, default='./experiments_deepgaze_dino_hybrid')
    parser.add_argument('--dataset_dir', type=str, default='./data/pysaliency_datasets')
    parser.add_argument('--lmdb_dir', type=str, default='./data/lmdb_caches_deepgaze_dino_hybrid')
