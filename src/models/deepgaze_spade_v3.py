...
This version uses a standard, non-SPADE fixation selection network, matching
the pattern of the other original scripts.
"""
import torch
...
from src.modules import (
    ...
    # IMPORTANT: Import the standard fixation selection builder
    build_fixation_selection_network, 
    encode_scanpath_features
)
...
try:
    ...
    raise ImportError("torch_scatter is required for the hybrid SPADE model.")


class DeepgazeSpadeV3(nn.Module):
    # The class definition is mostly the same...
    def __init__(self, densenet_features: FeatureExtractor,
                 ...
                 fixation_selection_network: nn.Module, # This is now a standard network
        ...
        self.fixation_selection_network = fixation_selection_network # Standard network
    ...
    def forward(self, image, centerbias, **kwargs):
        ...
        processed_features_list = [F.interpolate(f, size=readout_shape, mode='bilinear', align_corners=False) for f in densenet_feature_maps]
...
def build(cfg):
    ...
    densenet_features = FeatureExtractor(RGBDenseNet201(), [
    ...
    dino_features = DinoV2Backbone(
        layers=[extra.get("dino_semantic_layer_idx", -1)],
        model_name=extra.get("dino_model_name", "dinov2_vitb14"),
        freeze=True
    )
    semantic_path_channels = dino_features.num_channels
    ...
    if extra.get("is_scanpath_stage", False):
        ...
        # USE THE STANDARD, NON-SPADE FIXATION SELECTION NETWORK
    ...
    model = DeepgazeSpadeV3(
        ...
        fixation_selection_network=fixsel_net, # Now passing the standard one
